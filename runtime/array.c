...
typedef struct AnyArray {
    i32 length; 
    i8 debug_type;  // This is for debugging my compiler. Can be removed later since I trust javac to only emit well typed classfiles. 
} AnyArray;
...
#define generic_array(ty, dbg_type) \
    ...
    Array_objptr* array_fillmulti_##ty(i32 inner_len, Array_objptr* outer, i32 depth) { \
        assert(outer->h.debug_type == OBJ_TYPE_ID);                                     \
